...
function renderEventSlider($title, $events, $base_url)
{
    ...
    foreach ($events as $event) {
        ...
        $eventCards .= '
        <div class="flex-none w-80 bg-white border border-gray-200 rounded-lg shadow-sm p-4 max-h-screen overflow-hidden">
            ' . $imageContent . '
    ...
    //Nek gak onk opo opo (harus e pasti onk)
    if (empty($events)) {
        $eventCards = '
    ...
    <div>
        ...
        <div class="relative overflow-hidden">
            <div id="' . $sliderId . '" class="flex gap-4 flex-nowrap overflow-x-auto scroll-smooth pb-4 pr-4" style="scrollbar-width: none; -ms-overflow-style: none;">
            ' . $eventCards . '
            </div>

            <!-- Tombol Navigasi -->
            <button type="button" class="absolute left-0 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white border border-gray-300 rounded-full w-10 h-10 flex items-center justify-center shadow z-10 transition-all duration-200" onclick="scrollSlider(\'' . $sliderId . '\', -320)">
                <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                </svg>
            </button>

            <button type="button" class="absolute right-0 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white border border-gray-300 rounded-full w-10 h-10 flex items-center justify-center shadow z-10 transition-all duration-200" onclick="scrollSlider(\'' . $sliderId . '\', 320)">
                <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
